...
LOGGER = logging.getLogger('ytasmr')
...
def parse_arguments():
    ...
    Returns:
        args:  Argument dictionary
               (Type: dict[str, str])
    """
    parser = argparse.ArgumentParser(description='Download YouTube-ASMR data locally')
    ...
    parser.add_argument('url_file_path',
                        action='store',
                        type=str,
                        help='Path to YouTube-ASMR URL file')
    ...
    parser.add_argument('-lp',
                        ...
                        dest='log_path',
                        action='store',
                        default='yt-asmr-dl.log',
                        help='Path to log file generated by this script. ' \
                             'By default, the path is "./yt-asmr-dl.log".')
...
def ffmpeg(ffmpeg_path, input_path, output_path, input_args=None,
           output_args=None, log_level='error', num_retries=1,
    ...
    for attempt in range(num_retries):
        try:
            ...
            args = [ffmpeg_path] + input_args + inputs + output_args + [output_path]
            run_command(args)
    ...
    if video_mode != 'bestvideowithaudio':
        ...
        ffmpeg(ffmpeg_path, best_video_url, video_filepath,
               ...
               validation_args={'ffprobe_path': ffprobe_path,
                                ...
                                'end_past_video_end': end_past_video_end})
        # extract and reencode audio
        audio_output_args = ['-ar', str(audio_sample_rate),
                             '-vn',
                             '-ac', str(audio_info['channels']),
                             '-f', audio_format]
        ffmpeg(ffmpeg_path, video_filepath, audio_filepath, input_args=[], output_args=audio_output_args, num_retries=num_retries,
                validation_callback=None, validation_args={'audio_info': audio_info, 'end_past_video_end': end_past_video_end})
    else:
...
def download_ytasmr(data_dir, ffmpeg_path, ffprobe_path, url_file_path,
    ...
    start_time = timer()
    download_subset(url_file_path, data_dir, ffmpeg_path, ffprobe_path,
                    num_workers, **ffmpeg_cfg)
    end_time = timer()
    LOGGER.info('Download took %s seconds' % (end_time-start_time))
...
if __name__ == '__main__':
    ...
    download_ytasmr(**parse_arguments())
